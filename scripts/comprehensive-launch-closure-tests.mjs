...
assert.equal(essentialStatusCopy.registerLabel, undefined);
assert.match(essentialStatusCopy.nextStepDescription, /Essential report PDF/);
assert.doesNotMatch(essentialStatusCopy.nextStepDescription, /supporting register|final files/i);
...
assert.match(phase1, /complete_manual_report_generation/);
assert.doesNotMatch(phase1, /buildAndStoreSupportingRegister/);
const comprehensiveBranch = phase1.slice(phase1.indexOf('let storedRegister'));
assert.match(comprehensiveBranch, /if \(isComprehensive\)/);
assert.match(comprehensiveBranch, /storeVerifiedRegisterWorkbook/);
