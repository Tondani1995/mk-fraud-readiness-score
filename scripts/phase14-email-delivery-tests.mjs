...
const migration = read('supabase/migrations/0018_phase14_pdf_email_delivery.sql');
assert.match(migration, /provider_event_id text/i);
assert.match(migration, /email_events_provider_event_uidx/i);
assert.match(migration, /email_events_report_status_idx/i);
assert.match(migration, /attempt_number integer not null default 1/i);
...
console.log('Phase 14 PDF email delivery, idempotency, attachment and webhook tests passed.');
