...
select 'ledger_total_result|' || case when count(*) = 88 then 'PASS' else 'STOP' end
from supabase_migrations.schema_migrations;
select 'ledger_newest_result|' || case when max(version) = '20260805150000' then 'PASS' else 'STOP' end
from supabase_migrations.schema_migrations;
select 'preflight_ledger_boundary_result|' || case when
  ...
  and (select count(*) from supabase_migrations.schema_migrations where version > :'rc1_preflight_newest_version') = 54
...
select 'authorised_ledger_pairs_result|' || case when count(*) = 54 and bool_and(matches = 1) then 'PASS' else 'STOP' end
from found;
