...
select 'ledger_total_result|' || case when count(*) = 66 then 'PASS' else 'STOP' end
from supabase_migrations.schema_migrations;
select 'ledger_newest_result|' || case when max(version) = '20260803190000' then 'PASS' else 'STOP' end
from supabase_migrations.schema_migrations;
select 'preflight_ledger_boundary_result|' || case when
  ...
  and (select count(*) from supabase_migrations.schema_migrations where version > :'rc1_preflight_newest_version') = 32
...
with authorised(version, name) as (
  values
    ...
    ('20260801220000','rc1_synthetic_certification_marking'),
    ('20260802120000','rc1_synthetic_marking_placeholder_allowance'),
    ('20260803090000','rc1_certification_closure'),
    ('20260803160000','rc1_reactive_domain_content_correction'),
    ('20260803170000','rc1_reactive_domain_title_correction'),
    ('20260803180000','rc1_park_fulfilment_attempt'),
...
select 'authorised_ledger_pairs_result|' || case when count(*) = 32 and bool_and(matches = 1) then 'PASS' else 'STOP' end
from found;
...
select 'unlisted_post_preflight_result|' || case when not exists (
  ...
  where version > :'rc1_preflight_newest_version'
    and version not in ('20260722120000','20260722143000','20260724150000','20260724160000','20260724170000',
                        ...
                        '20260731130000','20260731150000','20260731170000','20260801070000','20260801090000','20260801120000','20260801140000','20260801160000','20260801180000','20260801200000','20260801220000','20260802120000','20260803090000','20260803160000','20260803170000','20260803180000','20260803190000')
) then 'PASS' else 'STOP' end;
