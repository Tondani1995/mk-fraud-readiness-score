...
select 'ledger_total_result|' || case when count(*) = 62 then 'PASS' else 'STOP' end
from supabase_migrations.schema_migrations;
select 'ledger_newest_result|' || case when max(version) = '20260803090000' then 'PASS' else 'STOP' end
from supabase_migrations.schema_migrations;
select 'preflight_ledger_boundary_result|' || case when
  ...
  and (select count(*) from supabase_migrations.schema_migrations where version > :'rc1_preflight_newest_version') = 28
...
select 'authorised_ledger_pairs_result|' || case when count(*) = 28 and bool_and(matches = 1) then 'PASS' else 'STOP' end
from found;
