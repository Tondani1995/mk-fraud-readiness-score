...
select 'ledger_total_result|' || case when count(*) = 47 then 'PASS' else 'STOP' end
from supabase_migrations.schema_migrations;
select 'ledger_newest_result|' || case when max(version) = '20260729170000' then 'PASS' else 'STOP' end
from supabase_migrations.schema_migrations;
select 'preflight_ledger_boundary_result|' || case when
  ...
  and (select count(*) from supabase_migrations.schema_migrations where version > :'rc1_preflight_newest_version') = 13
...
select 'authorised_ledger_pairs_result|' || case when count(*) = 13 and bool_and(matches = 1) then 'PASS' else 'STOP' end
from found;
