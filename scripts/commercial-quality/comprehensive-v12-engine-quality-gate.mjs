...
for (const profileKey of Object.keys(profiles)) {
  ...
  fs.writeFileSync(path.join(outDir, `${profileKey}.model-summary.json`), JSON.stringify({
    ...
    applicableQuestions: score.metrics.applicableQuestionCount,
    excludedQuestions: score.metrics.excludedQuestionCount,
    redirectedQuestions: score.metrics.redirectedQuestionCount,
    unknownQuestions: score.metrics.unknownQuestionCount,
    unansweredQuestions: score.metrics.unansweredQuestionCount,
    registerCounts,
    coreCounts,
    fingerprint
  }, null, 2) + '\n');
