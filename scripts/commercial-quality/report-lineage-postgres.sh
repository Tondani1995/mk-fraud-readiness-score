...
# macOS ships a locale that makes the postmaster multithreaded during startup
# ("postmaster became multithreaded during startup"), which stops the disposable cluster before it
# accepts a connection. A fixed C locale is sufficient and affects only this replay process.
...
replay_root="$(mktemp -d -t mk-report-lineage-pg.XXXXXX)"
...
cd "${release_root}"
MK_LINEAGE_POSTGRES_PORT="${replay_port}" node --experimental-strip-types --experimental-loader=./scripts/lib/ts-relative-resolve-loader.mjs scripts/commercial-quality/report-lineage-version-scope-regression.mjs
MK_LINEAGE_POSTGRES_PORT="${replay_port}" node --experimental-strip-types --experimental-loader=./scripts/lib/ts-relative-resolve-loader.mjs scripts/commercial-quality/affected-order-end-to-end-regression.mjs
