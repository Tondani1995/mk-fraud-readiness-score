...
-- One-time, controller-run UAT reconciliation. Never run against production.
...
-- equivalent, 0023-0031 -- and, as of Phase 1's 0032 service_role SELECT restore on
-- app_settings, 0032 too -- held out by the calling workflow step and reapplied only after
-- this reconciliation runs). 0032 is deliberately *not* listed here: it is not yet present
-- in the ledger at this point in either replay path, by construction of that hold-out.
...
begin
  if exists (select 1 from supabase_migrations.schema_migrations where version='0017' and name='phase14_canonical_disabled_foundation') then
    if to_regprocedure('public.terminal_phase14_generation_publication(jsonb,text,text)') is null
       or not exists (select 1 from information_schema.columns where table_schema='public' and table_name='phase14_security_gates' and column_name='authority_epoch') then
      raise exception 'phase14_canonical_ledger_schema_mismatch';
    end if;
    return;
  end if;
  select array_agg(version order by version) into v_actual from supabase_migrations.schema_migrations where version >= '0017';
  if v_actual is distinct from v_expected then raise exception 'phase14_uat_ledger_mismatch: expected %, received %', v_expected, v_actual; end if;
  if to_regclass('public.phase14_feature_policies') is null or to_regclass('public.phase14_ai_route_policies') is not null then
    raise exception 'phase14_uat_schema_boundary_mismatch';
  end if;
end;
$preflight$;
select exists (select 1 from supabase_migrations.schema_migrations where version='0017' and name='phase14_canonical_disabled_foundation') as phase14_already_reconciled \gset
\if :phase14_already_reconciled
  \echo 'Phase 14 UAT canonical reconciliation already committed; verified safe restart.'
\else
begin;
select set_config('lock_timeout','10s',true);
-- BEGIN REVIEWED UAT DELTA: 20260715022146_phase14_fifth_adversarial_remediation.sql
...
-- END REVIEWED UAT DELTA: 20260715022146_phase14_fifth_adversarial_remediation.sql
-- BEGIN REVIEWED UAT DELTA: 20260715073613_phase14_sixth_adversarial_remediation.sql
...
-- END REVIEWED UAT DELTA: 20260715073613_phase14_sixth_adversarial_remediation.sql
-- BEGIN REVIEWED UAT DELTA: 20260715073614_phase14_sixth_handoff_corrections.sql
...
-- END REVIEWED UAT DELTA: 20260715073614_phase14_sixth_handoff_corrections.sql
delete from supabase_migrations.schema_migrations where version in ('0018','0019','0020','0021','0022','20260714194317','20260714201550','20260714214023');
update supabase_migrations.schema_migrations set name='phase14_canonical_disabled_foundation', statements=array['Controlled UAT reconciliation: archived fifth and sixth deltas; SHA-256 a472e8d9a93052c8a51d2ec1cc2bc97a6b827a0b5fba97d3fdaa6f150ffab84b + f9589a09d28590728f84978129209f4e748ea1e248218fea78e036c2a09bff18 + d2ac47847dd764befae2772c18a44cd0e5427c034d5ac5d4d08717d3a1178d33'] where version='0017';
do $postflight$ begin if to_regprocedure('public.terminal_phase14_generation_publication(jsonb,text,text)') is null then raise exception 'phase14_uat_reconciliation_postflight_failed'; end if; end $postflight$;
commit;
\endif
select pg_advisory_unlock(hashtextextended('phase14-uat-canonical-reconciliation', 0));
