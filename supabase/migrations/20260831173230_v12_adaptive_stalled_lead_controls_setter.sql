...
begin
  ...
  -- Lock the single existing row before validating and replacing it. The
  -- fixed key is deliberately not a parameter, so this function cannot reach
  -- another app_settings entry.
  select s.value_json
    into v_current
  ...
  -- Reuse the existing closed control-plane audit context for the guarded
  -- app_settings/audit_logs writes. No table DML grant is added.
