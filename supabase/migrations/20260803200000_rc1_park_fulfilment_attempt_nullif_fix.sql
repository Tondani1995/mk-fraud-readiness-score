-- RC1: nullif is SQL syntax, not a function -- remove its schema qualification.
--
-- rc1_park_fulfilment_attempt refused every invocation with SQLSTATE 42883 (undefined_function)
-- on pg_catalog.nullif. nullif is a CASE shorthand resolved by the grammar, so it has no
-- pg_catalog entry and qualifying it fails regardless of search_path. Confirmed against the
-- catalogue: nullif, coalesce, greatest and least have no pg_proc rows, while btrim, char_length,
-- convert_to, encode, jsonb_build_object, now and left do and remain qualified.
--
-- This changes that one construct and nothing else. Privileges, the REPORT_QUEUED /
-- RETRY_SCHEDULED transition rule, the eligibility refusals, the audit branch, idempotency,
-- SECURITY DEFINER and search_path = '' are all unchanged, and the central freeze guard is not
-- touched.
...
begin
  ...
  set status = 'MANUAL_REVIEW_REQUIRED',
      ...
      quality_reviewed_by = nullif(v_actor->>'user_id', '')::uuid,
