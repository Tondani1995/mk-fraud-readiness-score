-- Align both paid products to the accepted Essential operating model.
--
-- This forward correction preserves payment verification, idempotency and the existing order
-- state machine. A paid transition records that an authorised MK operator must prepare and
-- manually deliver the selected report; it does not create a generation attempt, invoke a worker,
-- issue a customer token or send a customer email. Existing historical rows are untouched.
