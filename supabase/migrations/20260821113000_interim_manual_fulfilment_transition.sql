-- Interim paid-order fulfilment boundary.
--
-- Payment confirmation is still one atomic, idempotent state transition, but it must not create
-- a generation job or start a worker. An authorised MK operator starts preparation from the admin
-- controls after payment has been confirmed, then records the separate manual delivery step.
...
begin
  ...
  if p_new_state = 'PAID' then
    ...
    values (
      v_order.id,
      'manual_fulfilment_pending',
      'Verified payment recorded. An authorised MK operator must prepare, quality-check and manually deliver the purchased report.',
      pg_catalog.jsonb_build_object(
        'source', 'payment_confirmed',
        'technical_reference', p_technical_reference,
        'generation_requested', false,
        'automatic_delivery_requested', false
      )
    );
  end if;
  ...
  return pg_catalog.jsonb_build_object(
    ...
    'fulfilment', v_fulfilment_result, 'fulfilment_attempt_id', null
  );
exception when unique_violation then
  ...
  if found then
    return pg_catalog.jsonb_build_object(
      ...
      'fulfilment', 'NOT_REQUESTED', 'fulfilment_attempt_id', null
    );
  end if;
  raise;
end;
...
-- Invoice preference belongs to the same atomic order transaction. This wrapper delegates all
-- catalogue, price, idempotency and order-event rules to the frozen create_paid_order() RPC, then
-- records the optional billing request before either operation can commit.
create or replace function public.create_paid_order_with_invoice(
  p_tier text,
  ...
  p_invoice_details jsonb default '{}'::jsonb
) returns jsonb
language plpgsql
volatile
...
declare
  ...
  v_details jsonb := case
    when p_invoice_requested then coalesce(p_invoice_details, '{}'::jsonb)
    else '{}'::jsonb
  end;
begin
  v_result := public.create_paid_order(
    p_tier,
    p_assessment_id,
    p_expected_product_code,
    ...
    p_organisation_name,
    p_product_name,
    p_eft_instructions_snapshot,
    p_requested_by_respondent_id,
    p_assessment_reference
  );

  v_order_id := nullif(v_result->>'order_id', '')::uuid;
  if v_order_id is null then
    raise exception 'paid_order_invoice_binding_missing_order';
  end if;

  update public.orders
  set invoice_requested = p_invoice_requested,
      invoice_details = v_details,
      updated_at = now()
  where id = v_order_id;
  if not found then
    raise exception 'paid_order_invoice_binding_order_not_found';
  end if;

  return v_result || jsonb_build_object(
    'invoice_requested', p_invoice_requested,
    'invoice_details', v_details
  );
end;
$$;

comment on function public.create_paid_order_with_invoice(
  text, uuid, text, integer, text, uuid, text, text, text, text, jsonb, uuid, text, boolean, jsonb
) is
  'Atomic self-service paid-order creation with optional invoice details. Delegates the frozen product, price and idempotency contract to create_paid_order().';
revoke all on function public.create_paid_order_with_invoice(
  text, uuid, text, integer, text, uuid, text, text, text, text, jsonb, uuid, text, boolean, jsonb
) from public, anon, authenticated;
grant execute on function public.create_paid_order_with_invoice(
  text, uuid, text, integer, text, uuid, text, text, text, text, jsonb, uuid, text, boolean, jsonb
) to service_role;
