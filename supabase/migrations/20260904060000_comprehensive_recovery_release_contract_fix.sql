...
-- pre-empting it.
--
-- 20260904030000 introduced create_comprehensive_recovery_revision() and inserted the recovery
-- version already at status 'released', with its supporting register already 'released'. That
-- cannot work against the live contract: automatic_release_completed_fulfilment() requires the
-- report entering release to be 'generated' with VERIFIED storage, requires the register to be
-- 'verified' or 'released' so it can perform the release binding itself, and requires the attempt
-- to hold a live worker lease at REPORT_READY. A pre-released report would have been refused with
-- pdf_storage_verification_required, and nothing would have produced a delivery authorization --
-- so the revision would have had to manufacture one, bypassing the quality gates.
--
-- The seam now creates the version in the state the release contract expects and then calls that
-- contract inside the same transaction, so a recovery revision is released by exactly the same
-- gates as any other Comprehensive package: verified payment, locked score, PDF/Storage integrity,
-- commercial-quality evidence, single current version, delivery entitlement and recipient checks.
-- If release does not return released = true, this raises and the whole transaction rolls back,
-- leaving V1 released and current.
--
-- 20260904030000 is not edited. Only the seam is redefined here.
...
begin
  ...
  -- one. The release contract also requires exactly one live version for this assessment/type.
  -- Everything below is one transaction: if release does not succeed, V1 is restored to 'released'.
  update public.reports set status = 'superseded', updated_at = pg_catalog.now()
  where id = v_source.id;

  -- 'generated', not pre-released. The automatic quality release owns the transition to 'released'.
  ...
  -- 'verified', not pre-released. The release wrapper performs the release binding itself and
  -- raises comprehensive_package_release_binding_failed if it cannot.
  ...
  -- REPORT_READY holding a live bounded lease, which is what the release contract claims against.
  ...
  -- The existing automatic quality release, not a manufactured authorization. It owns the report
  -- transition to 'released', the register release binding, the email event, the delivery
  -- authorization and the attempt transition to DELIVERY_QUEUED.
...
-- The 12-argument shape from 20260904030000 pre-empted the release contract and must not remain
-- callable now that a correct seam exists.
