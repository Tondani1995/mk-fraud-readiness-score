...
export function AdaptiveAssessmentExperience({ assessmentReference, token, initialState }: { assessmentReference: string; token: string; initialState: AdaptiveState }) {
  ...
  async function chooseGateway(node: any, value: string) {
    ...
    await persist(next, controlResponses, 'gateway', node.nodeId);
  }
  ...
  async function chooseControl(node: any, response: { responseState: 'maturity' | 'unknown'; responseValue: number | null }) {
    ...
    await persist(gatewayAnswers, next, 'question', node.nodeId);
  }
  ...
  if (submitted || state.navigation?.current_screen === 'complete') return <Card><CardHeader><Badge>Adaptive assessment submitted</Badge><CardTitle className="mt-3">Your assessment is complete</CardTitle></CardHeader><CardContent className="space-y-4 text-sm leading-6 text-mk-muted"><p>The response set is locked to graph <strong>{state.graphVersion}</strong>. A numeric readiness score is not calculated in this product increment.</p><p>Applicable profile records saved: {state.path?.nodes?.length ?? 0}. Integrity signals recorded: {state.integritySignals?.length ?? 0}.</p><p className="font-medium text-mk-ink">Reference: {assessmentReference}</p></CardContent></Card>;
