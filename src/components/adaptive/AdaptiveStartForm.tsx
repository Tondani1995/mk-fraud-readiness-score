...
export function AdaptiveStartForm() {
  ...
  return <form onSubmit={submit} className="space-y-5">
    ...
    <div className="grid gap-5 sm:grid-cols-2">
      <label className="block text-sm font-medium text-mk-ink">Full name<input required value={form.fullName} onChange={(event) => update('fullName', event.target.value)} className="mt-2 min-h-12 w-full rounded-xl border border-mk-line bg-white px-4" /></label>
      <label className="block text-sm font-medium text-mk-ink">Work email<input required type="email" value={form.email} onChange={(event) => update('email', event.target.value)} className="mt-2 min-h-12 w-full rounded-xl border border-mk-line bg-white px-4" /></label>
    </div>
    <label className="block text-sm font-medium text-mk-ink">Organisation name<input required value={form.organisationName} onChange={(event) => update('organisationName', event.target.value)} className="mt-2 min-h-12 w-full rounded-xl border border-mk-line bg-white px-4" /></label>
