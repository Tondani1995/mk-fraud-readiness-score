...
export async function getAdminAssessmentDetail(assessmentReference: string, admin?: AdminSession) {
  ...
  const [scoreRunResult, domainResult, answerResult, exposureResult, traceResult, capResult, requestResult, auditResult] = await Promise.all([
  ...
  for (const [label, result] of Object.entries({ scoreRunResult, domainResult, answerResult, exposureResult, traceResult, capResult, requestResult, auditResult })) {
    if ((result as any).error) console.error(`admin assessment ${label} query failed`, (result as any).error);
  }
...
export async function getAdminMethodologyConfig() {
  ...
  const [domains, questions, exposureFactors, contentBlocks] = await Promise.all([
    activeMethodology?.id
      ? service.from('domains').select('domain_code,name,weight_pct,domain_type,is_core,sort_order').eq('methodology_version_id', activeMethodology.id).order('sort_order')
      : Promise.resolve({ data: [], error: null }),
    activeMethodology?.id
      ? service.from('questions').select('question_code,prompt,weight,is_critical,is_hard_gate,n_a_allowed,n_a_rule_key,active,sort_order,domains(domain_code,name,sort_order)').eq('methodology_version_id', activeMethodology.id).order('sort_order')
      : Promise.resolve({ data: [], error: null }),
    activeMethodology?.id
      ? service.from('exposure_factors').select('factor_code,name,max_points,input_type,options_json,sort_order').eq('methodology_version_id', activeMethodology.id).order('sort_order')
      : Promise.resolve({ data: [], error: null }),
    activeMethodology?.id
      ? service.from('report_content_blocks').select('block_key,block_type,domain_code,maturity_band,severity,title,status,version_number,updated_at').eq('methodology_version_id', activeMethodology.id).order('block_key')
      : Promise.resolve({ data: [], error: null })
  ]);
