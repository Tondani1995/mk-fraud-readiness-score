...
import { calculateAdaptiveReadinessScore } from '../scoring/adaptive-scoring';
import type { AssembledReportData, MaturityCapEventRecord, QuestionTraceRecord } from '../reports/types';

export const COMPREHENSIVE_V12_QUALITY_SOURCE_SHA = '8376242581772153c897687af3acd4da436509eb' as const;
export const COMPREHENSIVE_V12_QUALITY_METHODOLOGY = 'MFRS-V1.2-CANDIDATE-OWNER-CORRECTION' as const;
export const COMPREHENSIVE_V12_QUALITY_GRAPH = 'MFRS-V1.2-ADAPTIVE-CANDIDATE-20260821' as const;
export const COMPREHENSIVE_V12_QUALITY_GENERATED_AT = '2026-08-23T12:00:00.000Z' as const;
...
export type V12ProfileKey = keyof typeof profiles;
type Profile = (typeof profiles)[V12ProfileKey];
...
function valueFor(profile: Profile, nodeId: string, replacementFor: string | null): number {
...
function buildResponses(profile: Profile) {
  ...
  for (const node of path.activeNodes) {
    ...
    controlResponses[node.nodeId] = {
      ...
      responseValue: valueFor(profile, node.nodeId, node.replacementFor)
    };
  }
...
export function buildV12ProfileAssembled(profileKey: V12ProfileKey) {
  ...
  const data: AssembledReportData = {
    ...
    orderVerifiedAt: COMPREHENSIVE_V12_QUALITY_GENERATED_AT,
    ...
    generatedAt: COMPREHENSIVE_V12_QUALITY_GENERATED_AT,
