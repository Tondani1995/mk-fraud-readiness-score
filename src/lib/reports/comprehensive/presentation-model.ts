...
export function buildExecutivePresentationModel(model: ComprehensiveDeliveryModel): ExecutivePresentationModel {
  ...
  const slides: ExecutiveSlideContent[] = [
    ...
    { number: 2, title: 'Current readiness', objective: 'Show the deterministic position without overstating assurance.', keyMessage: `Readiness is ${Math.round(model.analytical.score.overallScore)}/100 at ${model.analytical.score.finalMaturity}, with ${Math.round(model.analytical.score.exposureScore)}/100 exposure.`, evidenceRefs: scoreRefs, suggestedVisual: 'score-position', speakerPrompt: 'Separate score movement from assurance movement.' },
