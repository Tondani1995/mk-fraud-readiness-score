...
export function buildExecutivePresentationModel(model: ComprehensiveDeliveryModel): ExecutivePresentationModel {
  ...
  const slides: ExecutiveSlideContent[] = [
    { number: 1, title: 'Why we assessed', objective: 'Set the decision context and scope.', keyMessage: 'This review moves from a recorded diagnostic to a target-state operating model and decision record.', evidenceRefs: [], suggestedVisual: 'headline', speakerPrompt: 'Confirm the decisions this session must enable.' },
    ...
    { number: 3, title: 'Recorded information and review notes', objective: 'Show the basis and its limitations.', keyMessage: `${model.validationSummary.validatedSupported} review note(s) sit alongside ${model.validationSummary.totalEvidenceItems} recorded information items; ${model.validationSummary.unresolved} remain open.`, evidenceRefs, suggestedVisual: 'validation-ledger', speakerPrompt: 'Challenge scope, completeness and limitations.' },
    { number: 4, title: 'Target-state operating model', objective: 'Make accountability, governance and escalation explicit.', keyMessage: 'The target state connects owners, forums, monitoring, incident response and separately scoped assurance.', evidenceRefs, suggestedVisual: 'validation-ledger', speakerPrompt: 'Resolve disagreements without rewriting the recorded answer.' },
    ...
    { number: 7, title: 'Implementation priorities', objective: 'Sequence action across 30/60/90 days and the following year.', keyMessage: 'Completion requires a named owner, a defined proof item and an effectiveness measure.', evidenceRefs: actionRefs, suggestedVisual: 'roadmap', speakerPrompt: 'Confirm dependencies and escalation thresholds.' },
    ...
    { number: 9, title: 'Next 90 days', objective: 'Close with near-term execution.', keyMessage: 'Close priority information gaps, establish the highest-risk controls and report exceptions.', evidenceRefs: model.roadmapActions.filter((action) => ['30 days', '60 days', '90 days'].includes(action.period)).flatMap((action) => action.evidenceRefs), suggestedVisual: 'roadmap', speakerPrompt: 'Agree the next checkpoint and the proof pack required.' },
    { number: 10, title: 'Closing / next steps', objective: 'Confirm the follow-through mechanism.', keyMessage: 'The board-ready output is a decision record plus a maintained target-state and traceability register.', evidenceRefs: [...actionRefs, ...evidenceRefs], suggestedVisual: 'closing', speakerPrompt: 'Read back decisions, owners, dates and oversight metrics.' }
  ];
  return {
    ...
    workshop: {
      ...
      agenda: [
        ...
        { order: 2, title: 'Information boundary', purpose: 'Surface gaps between recorded answers, supplied artefacts and review notes.', output: 'Scope and limitation decisions', durationMinutes: 25, linkedRefs: evidenceRefs },
      ...
      workingRules: [
        ...
        'Do not claim operating effectiveness beyond the information and scope recorded.',
