...
function friendlyHeader(column: string): string { return column.replace(/([a-z])([A-Z])/g, '$1 $2').replace(/^id$/i, 'Technical ID').replace(/^question code$/i, 'Question code (technical)').replace(/^source refs$/i, 'Source references (technical)').replace(/^proof ref$/i, 'Proof reference (technical)').replace(/^linked (finding|risk|control) ids?$/i, 'Linked $1 IDs (technical)').replace(/^./, (letter) => letter.toUpperCase()); }
function technicalColumn(column: string): boolean { return /(^id$|questionCode|sourceRefs|proofRef|linked(?:Finding|Risk|Control)Ids?)/i.test(column); }
...
function readMeRows(model: ComprehensiveDeliveryModel): Array<Record<string, string>> {
  return [
    ...
    { field: 'How to read', value: 'Start with Summary, then Findings and Risks, then Control Actions, Decisions and Roadmap.', note: 'Technical IDs and question codes support traceability.' },
    { field: 'Proof requirements', value: 'The Evidence Checklist records the proof management should retain while building and monitoring controls.', note: 'It is an implementation register, not an upload workflow.' },
...
export async function buildComprehensiveRegisterWorkbook(model: ComprehensiveDeliveryModel): Promise<ComprehensiveRegisterWorkbook> {
  ...
  const sheets = [
    ...
    ...registerSheets.filter((sheet) => sheet.name !== 'Question Traceability')
  ];
