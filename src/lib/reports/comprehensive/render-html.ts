...
function evidenceBadgeRow(item: EvidenceRequestPackItem): string {
  return `<div class="evidence-row"><div><strong>${short(item.evidenceRef)}</strong><div>${short(item.evidenceItem)}</div><small>${short(item.linkedDomain)} · Backend: ${short(item.backendStatus)}</small><small>Requested: ${short(item.requestedStatus)} · Conclusion: ${short(item.reviewerConclusion)}</small></div><div>${statusPill(item.validationStatus)}<small>Examined: ${short(item.actualArtefactsExamined.join('; '))}</small><small>Demonstrated: ${short(item.whatEvidenceDemonstrated ?? item.reviewerNote)}</small><small>Not demonstrated / limitation: ${short(item.whatEvidenceDidNotDemonstrate)}</small><small>Confidence: ${short(item.reviewerConfidence)}</small></div></div>`;
}

function findingCard(finding: ComprehensiveFindingView, index: number, compact = false): string {
  const details = compact
    ? `${field('Recorded response', finding.responseMeaning)}${field('Evidence reviewed', finding.evidenceRefsReviewed.join(', ') || 'None recorded')}${field('Reviewer interpretation', finding.adjustedInterpretation ?? finding.reviewerObservation)}${field('Evidence limitation', finding.evidenceLimitation)}`
    : `${field('Domain', finding.domainName)}${field('Recorded response', finding.responseMeaning)}${field('Why it matters', finding.whyItMatters)}${field('Evidence reviewed', finding.evidenceRefsReviewed.join(', ') || 'None recorded')}${field('Reviewer observation', finding.reviewerObservation)}${field('Reviewer interpretation', finding.adjustedInterpretation)}${field('Evidence limitation', finding.evidenceLimitation)}${field('Agreed owner / due date', [finding.agreedOwner, finding.agreedDueDate].filter(Boolean).join(' · '))}`;
...
export function renderComprehensiveReportHtml(model: ComprehensiveDeliveryModel): string {
  ...
  const pages = [
    ...
    page('controls', 'Control-design assessment', `<p class="lede">Control design is connected to the recorded finding and evidence question. Reviewer critique is shown beside, and does not mutate, the deterministic design.</p><table class="table"><thead><tr><th>Control action</th><th>Recorded state</th><th>Human design critique</th><th>Target / owner</th></tr></thead><tbody>${projection.controlActions.map((control) => { const review = model.controlDesignReviews.find((candidate) => candidate.controlId === control.id); return `<tr><td><strong>${short(control.id)}</strong><br>${short(control.controlObjective)}</td><td>${short(control.currentState)}<br>${short(control.controlDesign)}</td><td>${short(review?.designAssessment)}<br>${short(review?.designGapLimitation)}<br>${short(review?.recommendedAdjustment)}</td><td>${short(control.targetState)}<br>${short(control.accountableExecutive)} · ${short(control.targetPeriod)}</td></tr>`; }).join('')}</tbody></table>`),
    page('decisions', 'Leadership decisions', `<p class="lede">The Comprehensive product moves from diagnosis to decisions with traceable ownership. Options and trade-offs are shown only where the reviewer supplied them.</p>${model.leadershipDecisions.slice(0, 6).map((decision) => { const review = model.decisionReviews.find((candidate) => candidate.decisionId === decision.id); return `<article class="record"><div class="record-top"><span class="record-index">${short(decision.decisionCategory)}</span><span class="status-pill judgement">DECISION REQUIRED</span></div><h3>${short(decision.decisionRequired)}</h3><div class="record-grid">${field('Why now', decision.whyNow)}${field('Viable options', review?.viableOptions.join('; '))}${field('Key trade-offs', review?.keyTradeOffs.join('; '))}${field('Reviewer recommendation', review?.reviewerRecommendation)}${field('Management / board decision', review?.managementBoardDecision)}${field('Accountable executive', review?.owner ?? decision.accountableExecutive)}${field('Target date', review?.targetDate ?? decision.deadline)}${field('Immediate deliverable', decision.immediateNextDeliverable)}${field('Consequence of delay', decision.consequenceOfDelay)}${field('Evidence refs', decision.evidenceRefs.join(', '))}</div></article>`; }).join('') || callout('Decision log', 'No leadership decisions were supplied in the current review input.', 'amber')}`),
