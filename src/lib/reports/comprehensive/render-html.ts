...
export function renderComprehensiveReportHtml(model: ComprehensiveDeliveryModel): string {
  ...
  const pages = [
    ...
    page('validation', 'Evidence-validation summary', `<p class="lede">Validation is scoped to the items named in the evidence request pack. Evidence-item status and finding-level reviewer conclusion remain separate.</p>${sharedE3}<div style="margin-top:8mm">${projection.evidenceItems.map(evidenceBadgeRow).join('')}</div>`),
    page('movement', 'What the review changes', `<p class="lede">The deterministic readiness position remains locked; the review changes the confidence boundary around named evidence and actions.</p>${sharedE4}`),
    ...
    page('options', 'Decision options and trade-offs', `<p class="lede">Each decision is presented with distinct options so management can record cost, speed, ownership and control consequences explicitly.</p>${sharedE7}`),
