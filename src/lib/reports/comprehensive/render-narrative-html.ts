...
function domainProfile(chapter: ComprehensiveNarrativeChapter): string {
  if (!chapter.domainProfile.length) return '';
  return `<figure class="exhibit profile-exhibit">
...
function strengths(chapter: ComprehensiveNarrativeChapter): string {
  if (!chapter.strengths.length) return '';
  return `<figure class="exhibit positive-exhibit">
...
function sustainmentPriorities(chapter: ComprehensiveNarrativeChapter): string {
...
function findings(chapter: ComprehensiveNarrativeChapter): string {
...
function scenarios(chapter: ComprehensiveNarrativeChapter): string {
  if (!chapter.scenarios.length) return '';
  return `<figure class="exhibit watch-exhibit">
...
function controls(chapter: ComprehensiveNarrativeChapter, positive: boolean): string {
  if (!chapter.controls.length) return '';
  return `<figure class="exhibit ${positive ? 'positive-exhibit' : ''}">
...
function decisions(chapter: ComprehensiveNarrativeChapter): string {
...
function transformation(model: ComprehensiveNarrativePresentationModel, chapter: ComprehensiveNarrativeChapter): string {
  ...
  return `<figure class="exhibit ${model.narrativeMode === 'SUSTAINMENT' ? 'positive-exhibit' : ''}">
...
function chapterExhibits(model: ComprehensiveNarrativePresentationModel, chapter: ComprehensiveNarrativeChapter): string {
  ...
  return [
    domainProfile(chapter),
    strengths(chapter),
    sustainmentPriorities(chapter),
    findings(chapter),
    scenarios(chapter),
    controls(chapter, positive),
    decisions(chapter),
    transformation(model, chapter)
  ].filter(Boolean).join('');
}
...
function chapterHtml(model: ComprehensiveNarrativePresentationModel, chapter: ComprehensiveNarrativeChapter): string {
  ...
  return `<section class="chapter tone-${chapter.tone}" data-chapter="${esc(chapter.chapterId)}">
    <div class="chapter-marker">MK Fraud Readiness · Comprehensive · ${String(chapter.order).padStart(2, '0')}</div>
...
function css(): string {
  ...
  *{box-sizing:border-box}
  ...
  body{font:10.6pt/1.58 Arial,Helvetica,sans-serif;color:var(--mk-ink);background:#fff}
  .cover{height:260mm;page-break-after:always;background:var(--mk-navy-900);color:#fff;padding:24mm 20mm;display:flex;flex-direction:column;justify-content:space-between}
  .cover .eyebrow,.chapter-marker{text-transform:uppercase;letter-spacing:.13em;font-size:7.5pt;font-weight:700}
  .cover .eyebrow{color:var(--mk-brass)}
  ...
  .bar{height:3mm;background:var(--mk-neutral-100);border-radius:8px;overflow:hidden}
  ...
  .companion{break-before:page;background:var(--mk-neutral-bg);padding:12mm 10mm;margin-top:10mm;border-top:3px solid var(--mk-navy-700)}
  .companion h2{font-size:19pt}
  .companion p{max-width:155mm}
  .sheet-list{display:flex;flex-wrap:wrap;gap:2mm;margin-top:5mm}
  .sheet-list span{padding:2mm 3mm;border:1px solid var(--mk-rule);background:#fff;font-size:8pt}
...
export function renderComprehensiveNarrativeReportHtml(model: ComprehensiveNarrativePresentationModel): string {
  const cover = `<section class="cover">
    <div>
      <div class="eyebrow">MK Fraud Readiness · Comprehensive</div>
    ...
    <div class="cover-meta">Assessment ${esc(model.assessmentReference)} · Confidential</div>
  </section>`;
