...
export function runPreAiFactPackGates(pack: NarrativeFactPack, plan: NarrativeStoryPlan, writerBrief?: NarrativeWriterBrief): PreAiGateReport {
  ...
  const results: PreAiGateResult[] = [
    ...
    check('SCENARIO-FIELD-INTEGRITY', briefScenarios.length === 0 ? sparseHighReadiness : briefScenarios.every((scenario) => Boolean(scenario.currentControlWeakness && scenario.requiredControlResponse) && scenario.currentControlWeakness !== scenario.requiredControlResponse && !Object.prototype.hasOwnProperty.call(scenario, 'controlWeakness')), sparseHighReadiness ? 'No scenario is appropriate for this sparse high-readiness profile.' : 'Scenarios distinguish the recorded weakness from the required future control response.', sparseHighReadiness ? 'A sparse high-readiness profile contains an invalid scenario.' : 'A scenario uses the legacy field, has a blank field or conflates current weakness with future response.'),
