...
import {
  ...
  PREMIUM_REPORT_DEVELOPMENT_RECIPIENT
} from './premium-report-development-mode';
...
function mapDatabaseError(error: unknown) {
  ...
  const known = [
    ...
    'preview_development_delivery'
  ];
  const reason = known.find((candidate) => message.includes(candidate)) ?? 'delivery_initiation_failed';
...
export class PremiumReportAdminInitiationError extends Error {
  constructor(readonly reason: string, message: string, readonly status = 409) {
    super(message);
...
async function resolveOrder(db: any, input: PremiumReportAdminInitiationInput) {
  ...
  if (result.error || !result.data) throw new PremiumReportAdminInitiationError('order_not_found', 'The order could not be resolved.', 404);
...
async function resolveReport(db: any, input: PremiumReportAdminInitiationInput) {
  ...
  if (result.error || !result.data) throw new PremiumReportAdminInitiationError('report_not_found', 'The report could not be resolved.', 404);
...
async function deliverWithConcurrencyRecovery(db: any, reportId: string, recipient: string) {
  try {
    return await deliverPremiumReportEmail({
      reportId,
      actor: { actorType: 'system', action: 'admin_send' },
      recipientOverride: recipient,
      allowNonProductionTestOverride: true,
      developmentMode: true
    });
  } catch (error: any) {
    if (error?.code !== '23505' || !String(error?.message ?? '').includes('report_delivery_authorizations_one_active_uidx')) throw error;
    const { data: existing } = await db.from('report_delivery_authorizations')
      .select('status,email_event_id,provider_message_id,recipient_email,test_delivery')
      .eq('report_id', reportId).eq('recipient_email', recipient)
      .in('status', ACTIVE_DELIVERY_STATUSES).order('authorised_at', { ascending: false }).limit(1).maybeSingle();
...
export async function initiatePremiumReportDevelopmentDelivery(input: PremiumReportAdminInitiationInput) {
  assertPremiumReportDevelopmentMode();
  ...
  if (order.status !== 'payment_received') throw new PremiumReportAdminInitiationError('order_not_paid', 'The order is not payment_received.');
  if (!order.verified_at || !order.verified_by) throw new PremiumReportAdminInitiationError('payment_verification_missing', 'The order has no valid payment verification.');
  ...
  if (!['generated', 'approved', 'released'].includes(report.status)) throw new PremiumReportAdminInitiationError('report_status_ineligible', 'The report is not in a delivery-ready state.');

  const { count: transitionCount, error: transitionError } = await db.from('payment_transition_events')
    .select('id', { count: 'exact', head: true }).eq('order_id', order.id).eq('new_state', 'PAID').eq('processing_result', 'applied');
  ...
  try {
    ...
    if (error instanceof PremiumReportInitiationPolicyError) throw new PremiumReportAdminInitiationError(error.reason, error.message, error.status);
    throw error;
  }

  const { data: assessment, error: assessmentError } = await db.from('assessments').select('id,current_score_run_id').eq('id', order.assessment_id).maybeSingle();
  const { data: scoreRun, error: scoreRunError } = await db.from('score_runs').select('id,assessment_id,status,locked_at,input_hash').eq('id', report.score_run_id).maybeSingle();
  if (assessmentError || scoreRunError || !assessment || !scoreRun || scoreRun.assessment_id !== assessment.id
      || assessment.current_score_run_id !== scoreRun.id || scoreRun.status !== 'completed' || !scoreRun.locked_at || !SHA256.test(scoreRun.input_hash ?? '')) {
  ...
  try {
    assertReportAccessEligible({ report, currentReportId, expectedOrderId: order.id, purpose: 'email_delivery' });
  } catch (error) {
  ...
  if (!customerRecipient) throw new PremiumReportAdminInitiationError('recipient_missing', 'The order has no customer recipient.');
  const recipient = (input.recipient ?? customerRecipient).trim().toLowerCase();
  assertApprovedRecipient(recipient, recipientAllowlist());
  if (recipient !== PREMIUM_REPORT_DEVELOPMENT_RECIPIENT) {
  ...
  return {
    ...
    reusedExistingSend: result.reusedExistingSend, testDelivery: result.testDelivery,
