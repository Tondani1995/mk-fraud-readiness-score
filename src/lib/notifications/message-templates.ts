...
export function buildPaymentConfirmedMessage(input: PaymentConfirmedInput) {
  ...
  const text = `Hi ${name},\n\nWe have verified your payment for order ${input.orderReference}.\n\nAmount confirmed: ${amount}\nVerification date: ${verifiedDate}\n\nAn MK operator will now prepare and quality-check the purchased report. MK will email the final files manually to your nominated delivery email and mark the order delivered once that step is complete.\n\nQuestions? Contact us at ${SUPPORT_EMAIL}.\n\nRegards,\nMK Fraud Insights`;
  const html = `<p>Hi ${escapeHtml(name)},</p><p>We have verified your payment for order <strong>${escapeHtml(input.orderReference)}</strong>.</p><ul><li>Amount confirmed: ${escapeHtml(amount)}</li><li>Verification date: ${escapeHtml(verifiedDate)}</li></ul><p>An MK operator will now prepare and quality-check the purchased report. MK will email the final files manually to your nominated delivery email and mark the order delivered once that step is complete.</p><p>Questions? Contact us at ${escapeHtml(SUPPORT_EMAIL)}.</p><p>Regards,<br><strong>MK Fraud Insights</strong></p>`;
  return { subject, text, html };
}
