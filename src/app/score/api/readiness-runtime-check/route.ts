...
export async function GET(request: Request) {
  ...
  const resumeCheck = await validateResumeToken({ assessmentReference: started.assessmentReference, rawToken: token, consume: false });
  ...
  return NextResponse.json({
    ...
    databaseCounts: { domains: domains.count, questions: questions.count, exposureFactors: exposureFactors.count, adminProfiles: adminProfiles.count },
    startFlow: {
