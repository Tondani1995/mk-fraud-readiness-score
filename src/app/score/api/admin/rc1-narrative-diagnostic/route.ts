...
export async function POST() {
  ...
  if (Number(freeze.freeze_epoch) !== 21) return deny('RC1_DIAG_UNEXPECTED_EPOCH', 409);
  ...
  try {
    ...
    const MATURITY_BANDS = ['Reactive', 'Developing', 'Structured', 'Strategic'];
    const numbersIn = (text: string) => [...new Set((String(text ?? '').match(/-?\d+(?:[.,]\d+)?/g) ?? []))];
    const bandsIn = (text: string) => MATURITY_BANDS.filter((b) => new RegExp(`\\b${b}\\b`, 'i').test(String(text ?? '')));
    const collectNums = (value: unknown, out: Set<string>) => {
      if (typeof value === 'number') out.add(String(value));
      else if (typeof value === 'string') numbersIn(value).forEach((n) => out.add(n));
      else if (Array.isArray(value)) value.forEach((v) => collectNums(v, out));
      else if (value && typeof value === 'object') Object.values(value).forEach((v) => collectNums(v, out));
    };
    const evidenceById = new Map((evidence?.items ?? []).map((i: any) => [String(i.id), i]));

    // Resolve a validator path back to the narrative section it names. Structural only.
    const sectionFor = (rawPath: string) => {
      const n: any = narrative;
      const m = rawPath.match(/^(domainNarratives|gapCommentary)\[(\d+)\]/);
      if (m) return { section: n?.[m[1]]?.[Number(m[2])] ?? null, kind: m[1] };
      const base = rawPath.split('.')[0].split('[')[0];
      return { section: n?.[base] ?? null, kind: base };
    };

    const issues = (validation.issues ?? []).map((issue: any) => {
      const path = String(issue.path);
      const { section, kind } = sectionFor(path);
      const text = section ? `${section.title ?? ''} ${section.body ?? ''}` : '';
      const refs: string[] = Array.isArray(section?.evidenceRefs) ? section.evidenceRefs.map(String) : [];
      const available = new Set<string>();
      refs.forEach((r) => { const item = evidenceById.get(r); if (item) collectNums(item.value, available); });
      const detectedNumbers = numbersIn(text);
      const detectedBands = bandsIn(text);
      const domainItem = section?.domainCode ? evidenceById.get(`domain:${section.domainCode}`) : null;
      const expectedBand = (domainItem as any)?.value?.maturityBand ?? null;
      return {
        code: String(issue.code),
        path,
        sectionKind: kind,
        domainCode: section?.domainCode ?? null,
        evidenceRefs: refs,
        evidenceRefPrefixes: [...new Set(refs.map((r) => r.split(':')[0]))].sort(),
        numbersDetectedInSection: detectedNumbers,
        numbersAvailableViaCitedEvidence: [...available].sort(),
        unsupportedNumbers: detectedNumbers.filter((n) => !available.has(n)),
        expectedMaturityBand: expectedBand,
        maturityTermsDetected: detectedBands,
        rootCause: !section ? 'section_not_resolved'
          : String(issue.code).includes('metric') || String(issue.code).includes('numeric')
            ? 'numeric_claim_without_supporting_reference'
          : String(issue.code).includes('maturity')
            ? 'maturity_term_without_supporting_reference'
            : 'other_grounding_rule'
      };
    });
