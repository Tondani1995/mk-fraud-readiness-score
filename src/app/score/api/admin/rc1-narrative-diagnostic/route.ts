...
export async function POST() {
  ...
  // The database must be frozen with no active canary before any read runs.
  const { data: freeze } = await db
    .from('rc1_operation_freeze_state')
    .select('state,freeze_epoch,active_canary_authorization_hash,active_canary_expires_at')
    .maybeSingle();
  if (!freeze || String(freeze.state).toUpperCase() !== 'FROZEN') return deny('RC1_DIAG_REQUIRES_FROZEN', 409);
  if (freeze.active_canary_authorization_hash || freeze.active_canary_expires_at) {
    return deny('RC1_DIAG_CANARY_ACTIVE', 409);
  }
