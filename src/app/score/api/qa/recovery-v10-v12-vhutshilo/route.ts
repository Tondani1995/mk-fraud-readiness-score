...
function normaliseRecoveryCustomerHtml(html: string): string {
  let cleaned = html;
  ...
  if (/\bSCENARIO-\d+\b|\(D\d+\)|the relevant control|the relevant roadmap action|authoritative roadmap dependency IDs|\bERP\b|personal-data obligations may be breached|majority of activity is never examined/i.test(cleaned)) {
