...
import {
  ...
  profiles,
  THREE_PROFILE_ENGINE_SOURCE_SHA,
  THREE_PROFILE_V12_GRAPH,
  type V12ProfileKey
} from '@/lib/qa/v12-three-profile-fixture';
...
const ENGINE_SOURCE_SHA = THREE_PROFILE_ENGINE_SOURCE_SHA;
...
function safeName(value: string): string {
  return value.replace(/\s*\(Pty\) Ltd\s*/i, '').replace(/[^A-Za-z0-9]+/g, '_').replace(/^_+|_+$/g, '');
}
...
function tarHeader(name: string, size: number): Buffer {
  ...
  header.fill(0x20, 148, 156); put('0', 156, 1); put('ustar\0', 257, 6); put('00', 263, 2);
...
function deterministicMeta(profileKey: V12ProfileKey) {
  const { data, score, path, controlResponses } = buildV12ProfileAssembled(profileKey);
  return {
    sourceSha: ENGINE_SOURCE_SHA,
    graphVersion: THREE_PROFILE_V12_GRAPH,
    profile: profileKey,
    organisationName: data.organisationName,
    assessmentReference: data.assessmentReference,
    reportReference: data.reportReference,
    inputHash: data.scoreRun.inputHash,
...
async function generateComprehensiveInterpretationViaOidc(
  ...
  const maxRepairs = options?.maxRepairsPerSlot ?? 0;
  const timeoutMs = options?.timeoutMs ?? 240_000;
  const accounting = {
    calls: 0, repairs: 0, inputTokens: 0, outputTokens: 0, totalTokens: 0, costMicros: 0,
    durationMs: 0, model, repairedSlots: [] as InterpretationSlotId[]
  };
  const startedAt = Date.now();
  const call = async (prompt: string): Promise<Record<string, unknown>> => {
    accounting.calls += 1;
    const response: any = await generateText({
      model, system: SYSTEM, prompt, output: Output.text(), maxOutputTokens: 6_000, maxRetries: 0,
      providerOptions: { gateway: { only: [provider] } }, abortSignal: AbortSignal.timeout(timeoutMs)
    });
  ...
  for (let attempt = 1; attempt <= maxRepairs; attempt += 1) {
    ...
    const prompt = [
      buildInterpretationPrompt(brief, failing), '', '================ REPAIR ================',
      ...
      reasons, '', 'PREVIOUS TEXT:', JSON.stringify(Object.fromEntries(failing.map((slot) => [slot, current[slot] ?? '']))), '',
...
async function renderComprehensiveTar(profileKey: V12ProfileKey): Promise<Buffer> {
  const { data: essentialData } = buildV12ProfileAssembled(profileKey);
  const reportReference = essentialData.reportReference.replace(/-V1$/, '-COMPREHENSIVE-V1');
  const data = {
    ...essentialData,
    reportReference,
    ...
    productPriceCents: 3_500_000
  } as typeof essentialData;
  ...
  const result = await renderComprehensiveReportPackage({
    ...
    generateInterpretation: async (brief, options) => {
      const run = await generateComprehensiveInterpretationViaOidc(brief, { ...options, model: COMPREHENSIVE_MODEL, maxRepairsPerSlot: 0 });
  ...
  if (!acceptedValidation || !acceptedValidation.publishable) throw new Error('Comprehensive final five-layer validation did not reach publishable acceptance.');
  const meta = {
    ...deterministicMeta(profileKey), reportReference, tier: 'Comprehensive',
  ...
  const base = safeName(data.organisationName);
  return buildTar([
    { name: `${base}_Comprehensive_V1_2.pdf`, bytes: result.pdf },
    { name: `${base}_Comprehensive_V1_2.xlsx`, bytes: result.workbook.bytes },
    { name: `${profileKey}-comprehensive-meta.json`, bytes: Buffer.from(JSON.stringify(meta, null, 2), 'utf8') }
  ]);
}

export async function GET(_request: Request, props: { params: Promise<{ profile: string; artifact: string }> }) {
  const { profile: rawProfile, artifact } = await props.params;
  if (!(rawProfile in profiles)) return NextResponse.json({ ok: false, error: 'unknown_profile' }, { status: 404 });
  const profileKey = rawProfile as V12ProfileKey;
  try {
    if (artifact === 'meta') {
      return NextResponse.json({
        ok: true, ...deterministicMeta(profileKey),
    ...
    if (artifact === 'comprehensive') {
      const tar = await renderComprehensiveTar(profileKey);
      return new Response(new Uint8Array(tar), {
        status: 200,
        headers: {
          'Content-Type': 'application/x-tar',
          'Content-Disposition': `attachment; filename="${profileKey}-comprehensive-package.tar"`,
    ...
    console.error('v12_three_product_pair_failed', { profile: profileKey, artifact, message });
    return NextResponse.json({ ok: false, profile: profileKey, artifact, error: message.slice(0, 4000) }, { status: 500, headers: { 'Cache-Control': 'no-store' } });
  }
}
