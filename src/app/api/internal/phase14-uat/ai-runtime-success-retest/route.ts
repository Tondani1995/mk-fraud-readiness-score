...
const AUTHORISATION_KEY = 'phase14_uat_ai_runtime_retest';
const SCENARIO = 'success_clean';
const ORDER_REFERENCE = 'MKORD-UAT-AI3-SUCCESS';
const FULFILMENT_ID = '80d32d85-1ad4-4ac2-b147-97a04bb801b8';
...
export async function GET() {
  ...
  const { data: fixtureState, error: fixtureError } = await db
    ...
    .eq('id', FULFILMENT_ID)
    .eq('orders.order_reference', ORDER_REFERENCE)
    .maybeSingle();
  ...
  try {
    const generator = createAiSdkPremiumReportNarrativeGenerator(flags.model);
    ...
    const { data: runs, error: runsError } = await db
      ...
      .eq('fulfilment_id', FULFILMENT_ID)
    ...
    await db.from('app_settings').update({
      value_json: {
        ...
        success_clean_completed_at: new Date().toISOString(),
        success_clean_report_id: result.reportId,
        success_clean_mode: result.generationMode
      }
    }).eq('setting_key', AUTHORISATION_KEY);
    ...
    const modeMatched = result.generationMode === 'ai';
    return respond({
      ok: modeMatched,
      scenario: SCENARIO,
      expectedMode: 'ai',
    ...
    try {
      await db.from('app_settings').update({
        value_json: {
          ...
          success_clean_failed_at: new Date().toISOString(),
          success_clean_error: error instanceof Error ? error.message : 'unknown_error'
        }
      }).eq('setting_key', AUTHORISATION_KEY);
    } catch {
