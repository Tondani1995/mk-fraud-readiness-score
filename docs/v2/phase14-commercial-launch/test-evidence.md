...
| `scripts/phase14-delivery-reconciliation-tests.mjs` (`phase14:test-delivery-reconciliation`) | H4 (17 cases, incl. strengthened test #4 direct database-invariant assertions) | Pass — 20/20 consecutive runs after the migration `0031` precision fix (previously ~2 of 3 runs failed; see known-risks doc, now moved to "Resolved this pass") |
