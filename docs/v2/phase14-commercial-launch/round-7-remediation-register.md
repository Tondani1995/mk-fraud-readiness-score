...
| H6 | No documented evidence of the production migration-0017 activation boundary, and no operator runbook | **Fixed** | This session. See `docs/v1/phase14/production-history-read-only-evidence-2026-07-15.md` (read-only production schema/migration-ledger capture, SHA-256 `417dfbf2fb...`) and `docs/v1/phase14/migration-0017-note.md` for the existing evidence; the operator runbook is `docs/v2/phase14-commercial-launch/production-activation-runbook.md` (new, this session). This branch's first real CI run against Docker/Supabase CLI (never previously exercised in this sandbox) surfaced that `.github/workflows/supabase-migration-replay.yml`'s fresh-vs-upgraded schema-equivalence check (M14's own drill) genuinely fails: `fresh_hash` != `upgrade_hash`. Not yet root-caused; the workflow now persists both raw inventories and an auto-generated diff to its evidence artifact for the next run. **This specific CI gate is not yet green** -- see M14's own entry below and `known-risks-and-launch-limitations.md`'s H6/M14 section for full detail and the action item. H6 itself (the documented evidence and operator runbook, which is what this row covers) is unaffected and remains fixed. |
...
| M14 | Migration-ledger reconciliation drill exists but has no operator runbook | **Runbook fixed; drill itself currently red** | The operator runbook is `docs/v2/phase14-commercial-launch/production-activation-runbook.md` (new, this session) -- that part of the finding is closed. The drill's own fresh-vs-upgraded schema-equivalence check, however, genuinely fails on this branch's first real CI run (not yet root-caused; see `known-risks-and-launch-limitations.md`). This finding cannot be marked fully closed until that check passes green. |
