# Integrated A-D Release Candidate — RC0 Discovery Plan (Revision 4)

**Current reconciliation status (2026-07-27):** the controller-selected migration-history designs
are implemented on the RC branch. The fresh disposable replay is green; the complete CI battery is
still pending, so **RC PREPARATION: IN PROGRESS**. The prior Revision 3 blocker report is retained below as history;
it is not the current result. The four reconciliation categories are now explicit: exact
`applied-history/` archives, canonical active replay migrations, retired never-applied consolidated
migrations, and the seven genuinely pending migrations.
...
| State | Status |
|---|---|
| RC PREPARATION | **IN PROGRESS** — replay green; complete CI battery and controller review pending |
...
| This document's decision status | **RC PREPARATION: IN PROGRESS. RC MIGRATION/DEPLOYMENT, CLOUD CERTIFICATION, PUBLIC LAUNCH: NO-GO.** See §11/§12. |
...
### RC PREPARATION — IN PROGRESS

**Authorised scope; completion pending.** Permits only:
...
**RC PREPARATION: IN PROGRESS. RC MIGRATION/DEPLOYMENT: NO-GO. CLOUD CERTIFICATION: NO-GO.
PUBLIC LAUNCH: NO-GO. DO NOT MERGE.**

RC MIGRATION/DEPLOYMENT remains NO-GO specifically because the controller has not yet reviewed a
successful fresh replay and green CI on the new final head. The two earlier blockers are retained as
the Revision 3 history: `0011` is now retired from canonical replay and the real
`20260709033522` template seed is restored; active `0009` now deterministically allocates the
Production `MFRS-V1.1` UUID while the exact cloud SQL remains separately archived. Until replay and
all six workflows are green, RC preparation is not code-complete.
