...
| `0009` | `0009_methodology_copy_polish` | `0009_methodology_copy_polish.sql` | unchanged |
...
| `20260709033522` | `phase10_v2_report_template_seed` | **none — see §3 (deliberately not restored, open conflict)** | still none |
...
## 3. Deliberately not restored: `20260709033522` — confirmed conflict with `0011`
...
Neither option is implemented. `report_templates` seed content is not touched by anything in this
reconciliation beyond documenting the conflict.
...
**Result: the reconciled migration set does not currently replay cleanly.**
