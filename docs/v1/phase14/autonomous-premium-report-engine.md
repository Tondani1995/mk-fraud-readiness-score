...
AI SDK 6.0.83 is pinned because it supports the current Node 24 runtime. Structured output uses `generateText` with `Output.object` and a strict Zod schema. The Essential model policy is configurable through the documented database/environment precedence, with compiled default `openai/gpt-5-mini` and the ordered technical fallback chain `openai/gpt-5.6-luna` → `openai/gpt-5.6-terra` → `openai/gpt-5.6-sol`. Application-owned retries are bounded at four total Mini attempts per logical stage; SDK retries are disabled.
